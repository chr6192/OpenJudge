"""
Test code examples from docs/building-graders/custom-graders.md
"""
import pytest
import re
import json
...
from rm_gallery.core.graders.schema import GraderScore, GraderRank
    ...
    async def test_length_evaluator(self):
        ...
        async def length_evaluator(query: str, answer: str) -> GraderScore:
            ...
            return GraderScore(
                name="length_grader",
                score=score,
                reason=f"Length: {length} chars (target: 100+)"
            )
        
        grader = FunctionGrader(
            func=length_evaluator,
            name="length_check",
            mode="pointwise"
        )
    ...
    async def test_length_ranker(self):
        ...
        async def length_ranker(query: str, answer_1: str, answer_2: str) -> GraderRank:
            ...
            return GraderRank(
                name="length_ranker",
                rank=rank,
                reason=f"Lengths: {lengths[0]} vs {lengths[1]} chars"
            )
...
class TestComplexLogicGraders:
    ...
    async def test_regex_pattern_grader(self):
        ...
        result = await email_checker.aevaluate(
            response="Contact us at test@example.com"
        )
    ...
    async def test_content_quality_grader(self):
        ...
        class ContentQualityGrader(BaseGrader):
            ...
            async def aevaluate(self, response: str, **kwargs) -> GraderScore:
                ...
                # Generate reason
                reasons = [
                    f"Length: {len(response)} chars ({'✓' if length_ok else '✗'})"
                ]
    ...
    async def test_custom_grader_validation(self):
        ...
        async def simple_math_checker(query: str, response: str) -> GraderScore:
            ...
            return GraderScore(
                name="math_checker",
                score=score,
                reason="Correct answer" if score == 1.0 else "Incorrect or unknown"
            )
