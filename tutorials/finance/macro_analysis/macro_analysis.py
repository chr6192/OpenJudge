...
a. Multi-dimensional analysis:
    ⅰ. Time dimension: Reflect the temporal nature of financial event impacts, distinguish between short-term and long-term impacts, show phased evolution characteristics
...
DEFAULT_MACRO_ANALYSIS_TEMPLATE = PromptTemplate(
    messages={
        LanguageEnum.EN: [
            ChatMessage(
                role="user",
                content=textwrap.dedent(MACRO_ANALYSIS_PROMPT_EN),
            ),
        ],
        LanguageEnum.ZH: [
            ChatMessage(
                role="user",
                content=textwrap.dedent(MACRO_ANALYSIS_PROMPT_ZH),
            ),
        ],
    },
)
...
class MacroAnalysisGrader(LLMGrader):
    ...
    Evaluates the quality of macroeconomic analysis by comparing two responses based on
    completeness (transmission mechanism), depth (multi-dimensional, quantitative/qualitative,
    case studies), and logical consistency.

    Evaluation Criteria:
    1. Completeness: Clear transmission mechanisms and causal chains
    2. Depth: Time dimension, multi-level analysis, quantitative/qualitative combination, cases
    ...
    def __init__(
        ...
        template: Optional[PromptTemplate] = DEFAULT_MACRO_ANALYSIS_TEMPLATE,
        ...
        self.template = template if template is not None else DEFAULT_MACRO_ANALYSIS_TEMPLATE
    ...
    async def aevaluate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            logger.error(f"Error evaluating macro analysis: {e}")
            rank = [1, 2]
